            ...
            with st.spinner("🔗 Generating embeddings and saving to vector store..."):
                ...
                try:
                    vector_store = Qdrant.from_documents(
                        ...
                        url=qdrant_url,
                        api_key=qdrant_api_key,  # Added Qdrant API key
...
3. Use the chat to interact with the document.
"""
)
...
if st.session_state.chat_ready:
    ...
    try:
                ...
                if results:
                    ...
                    try:
                        response = client.chat.completions.create(
                            model="gpt-4.1",  # Verify model name
                            messages=[
        ...
        st.error(f"❌ Failed to connect to Qdrant collection: {e}")
        st.stop()